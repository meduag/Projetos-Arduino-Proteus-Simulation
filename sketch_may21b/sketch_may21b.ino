...
void serial_api_get(){
  ...
  if (Serial.available() > 0) {
    ...
    // process api
                if(serial_in == S_READ_ADS )    {read_ads_data = true; valid_cmd = true;} 
    if(serial_in == S_STOP_READ_ADS ) {read_ads_data = false; valid_cmd = true;} 
    ...
    if(serial_in == S_STOP_SEND_SERIAL )    {serial_send_data = false;  digitalWrite(PIN_LED, LOW); valid_cmd = true; }
                if (valid_cmd == false)                 {Serial.println("Invalid Command --> "); Serial.println(serial_in); Serial.println(" <--"); }
                valid_cmd == false;
  }
}
