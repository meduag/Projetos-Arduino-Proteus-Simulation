...
SoftwareSerial Serial_ESP =  SoftwareSerial(rxPin, txPin);

const int Btn1 = 4; // Boton Light 1 > Principal
const int Btn2 = 5; // Boton Light 2 > Corredor
const int Btn3 = 6; // Boton Light 3 > Banho

const int time_button = 500;

// Array de pinos de salida
int Output_Pins[3] = {Lg1, Lg2, Lg3};

// Array de pinos de Entrada
int Input_Pins[3] = {Btn1, Btn2, Btn3};

// Variables para tratar mensages
int legnth_In = 0, legnth_Out = 0, legnth_bvar = 0;

// Mensaje base
String msn_base = "0>0>0>";

// Led IR en el pino 3 para controlar TV e Aire acondicionado
IRsend IrSender;  
...
void lee_datos_de_ESP() {
  ...
  if (Serial_ESP.available() != 0) {
    ...
    for (int i = 0; i < legnth_bvar; i++) {
      data_in = Serial_ESP.readStringUntil(fin);
    ...
    // Configuro los pinos de salida de las luces  >> cabiar opcion para mudar el pino que cambioó de estado
    for (int i = 0; i < legnth_Out; i++) {
