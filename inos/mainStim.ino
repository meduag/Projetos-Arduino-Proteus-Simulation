...
// Types
#define COMM_REQUISITION_START_BYTE   0x23
#define COMM_RESPONSE_START_BYTE      0x24
#define COMM_NOTIFICATION_START_BYTE  0x25
#define COMM_CONFIRMATION_START_BYTE  0x26

// Sub types
#define COMM_RT_STOP_ALL                      0xF0
#define COMM_RT_STOP_CHANNEL                  0xF1
...
#define INTERCHANNEL_DELAY  500 //ms
...
void startRheobase()
{
  ...
  while (!d.is_contracted)
  {
    // increase amplitude
    for (iterator = 0; iterator < d.num_channels; iterator++)
      d.amplitudes[iterator] += 0.05; // us
      
    // stimulate
    Serial.println("Amplitude: ");
    Serial.println(d.amplitudes[0]);
...
void startChronaxie()
{
  ...
  while (!d.is_contracted)
  {
    ...
    for (iterator = 0; iterator < d.num_channels; iterator++)
      d.pulse_widths[iterator] += 10; // ms
      
    // stimulate
    Serial.println("Pulse width: ");
    Serial.println(d.pulse_widths[0]);
...
void startAccommodation()
{
  ...
  while (!d.is_contracted)
  {
    ...
    Serial.println("Streaming...");
    for (iterator = 0; iterator < d.num_channels; iterator++)
      d.measured_data[iterator] = d.amplitudes[iterator]*4.096;
    streamDiagnosisMessage(true);
  }
