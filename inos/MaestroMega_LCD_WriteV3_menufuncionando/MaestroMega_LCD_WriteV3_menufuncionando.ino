...
int Lim_T1 = 1000, Lim_T2 = 1000, Lim_T3 = 1000, Lim_T4 = 1000, Lim_mA = 100, Lim_Fq = 1000;

int T1 = 500, T2 = 0, T3 = 0, T4 = 0, mA = 1, Fq = 250, canal = 0;
int aumento = 5;

char* Menu_Pro[] = {"Electro Diagnost", "Terapeutica     ", "Actualizar dato "};
...
{
  pinMode(act, OUTPUT);
  pinMode(activ, OUTPUT);//para ativar o relay
      ...
      case btnSELECT:
        {
          ...
          if (Opc1.equals(Opc2)) {
            //Llamar al funcion de envio de señal actualizar datos
            Atualizar_Dato() ;
          }
...
void Atualizar_Dato() {
  ...
  ///Envia la sequencia de datos


  digitalWrite(act , HIGH);//interrupcion externa
  //Serial.println("\n Nivwl Alto  ");
  delay(100);
  digitalWrite(act , LOW);
  //Serial.println("\n Nivwl Bajo  ");
  ...
  digitalWrite(activ , LOW);//Desactiva el canal
...
void wireDado(int ICar) {
  String str;  //declaring string
  str = String(ICar); //converting integer into a string
  char b[sizeof(str)];   //declaring character array
  str.toCharArray(b, sizeof(str)); //passing the value of the string to the character array
  Wire.write(b);//
}

void enviarDados(int canalP) {

  digitalWrite(activ , LOW);//desactiva el canal

  //lcd.clear();
  //lcd.setCursor(0, 0);
  //lcd.print("Enviando Canal_1");

  ///Envia la sequencia de datos haciendo interrupcion externa
  digitalWrite(act , HIGH);
  delay(100);
  digitalWrite(act , LOW);

  Serial.println("\n Canal  ");
  Serial.println(canalP);

  Wire.beginTransmission(canalP);
  Wire.write("Q");
  wireDado(T1);
  Wire.write(";");
  wireDado(T2);
  ...
  delay(100);
  digitalWrite(activ , HIGH);//Activa el canal
